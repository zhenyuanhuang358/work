...
# ── 配置：改成你的报告标题 ─────────────────────────────────
REPORT_TITLE = "Jev 白皮书 · 只做选择题的模型 · 2026-09"
...
# key 必须与 build.py 里 toc_row 的第一个参数逐字相同（含全角空格），patch_toc 用它做正则回填。
# ⚠ pat 里不要写章号：h2 是 flex 布局，章号 span 与标题是两个 flex item，
#   pdftotext 会把它们拆到不同位置，带号的正则一个也匹配不到（2026-09-19 实测）。
ANCHORS = [
    ("00　摘要", r"^\s*摘要\s*$"),
    ("01　界定：一个只做选择题的模型", r"界定：一个只做选择题的模型"),
    ("02　官方主张与它们的口径", r"官方主张与它们的口径"),
    ("03　生态实况：217 个项目的可用性分布", r"生态实况：217 个项目的可用性分布"),
    ("04　四次失败的结构解剖", r"四次失败的结构解剖"),
    ("05　两轮实测的证据强度复核", r"两轮实测的证据强度复核"),
    ("06　一次独立基准的构建", r"一次独立基准的构建"),
    ("07　机制：为什么只有一种用法站得住", r"机制：为什么只有一种用法站得住"),
    ("08　局限与替代解释", r"局限与替代解释"),
    ("09　结论与决策框架", r"结论与决策框架"),
    ("附录 A　参考文献与来源", r"参考文献与来源"),
    ("附录 B　数据速查页", r"数据速查页"),
]
...
def locate(pdf, anchors):
    ...
    for p in sorted(pages):
        for key, pat in anchors:
            ...
            if p <= toc_page and "摘要" not in key:
                continue
