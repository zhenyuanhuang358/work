...
## 答案前置原则（2026-08-06 用户投诉「精力涣散」后确立 · 优先级高于其他排版规则）

**用户要的是判断。正文交付判断，末尾附注交付严谨。**

- **每个分析段落第一句必须是判断本身**，不是"关于这一点需要先说明数据口径"
- **正文禁止出现**：数据来源限制、抓取失败说明、"我上一版说错了"、逐句置信度散文
- 倒算 / 推算值：条目末尾一个徽章即可，**不在正文解释推算过程**（过程放附注）
- **报告末尾必须有「分析师附注 / Analyst's Note」板块**：口径与数据限制、推算明细、版本修订说明、**分析师独立看法与分歧点**、待核实项
- 版本修订（如 v1→v2 的判断更正）：**只在附注里写**，正文直接呈现修订后的判断，不在正文自我更正
