...
def get_nvda_price() -> str:
    if STOCK_PRICES_FILE.exists():
        try:
            data = _jsonsafe.loads(STOCK_PRICES_FILE.read_text(encoding="utf-8"),
                                   "stock_prices.json")   # 1.1u
...
def main():
    ...
    QUARTER_FILE.write_text(quarter)
    HASH_FILE.write_text(content_hash)

    price = get_nvda_price()
    if price:
        print(f"NVDA price from cache: ${price}")
        Path("nvda_price.txt").write_text(price)
    else:
        Path("nvda_price.txt").write_text("")
