...
def main():
    ...
    quadrant = _load_or_run(
        str(inter_dir / "s1_quadrant.json"),
        lambda: run_quadrant_analysis(category, core_target, user_focus),
    )
    scenarios = _load_or_run(
        str(inter_dir / "s2_scenarios.json"),
        lambda: run_scenario_analysis(category, core_target, quadrant),
    )
    lifecycle = _load_or_run(
        str(inter_dir / "s3_lifecycle.json"),
        lambda: run_lifecycle_analysis(category, core_target, scenarios),
    )
    breakthrough = _load_or_run(
        str(inter_dir / "s4_breakthrough.json"),
        lambda: run_breakthrough_analysis(category, core_target, lifecycle, scenarios),
    )
