...
# 已移出候选池（记录理由，避免日后重复放回）:
#   NU (Nu Holdings) — 2026-08-19 移出。数字银行没有毛利率/营业利润率的常规口径
#   （收入侧是净息差与手续费，成本侧是信用损失与资金成本，不存在销货成本），
#   本筛三个判别器全部失效。它的 SES 假设（低费率换用户规模）可能完全成立，
#   但必须换一套指标验证：净息差趋势 / 手续费率趋势 / 获客成本 vs 单客户终身价值。
#   见 references/failure-modes.md F9。
#
#   SE (Sea Limited) — 2026-08-19 移出，判非 SES 置信 9.5/10。
#   Shopee 抽佣率 12.39%(FY24) → 13.03%(FY25)，收入增速 33.4% > GMV 增速 26.8%；
#   2025 平台费 4%→10%、马来跨境佣金 16.2%→18.36%，2026-01 基础佣金在多数品类翻倍。
#   毛利率 +21.4pct、营业利润率 +39.9pct 是撤补贴的算术，不是成本优势。F1 成立。
#   ⚠ 原注释「游戏毛利远高于电商会掩盖 Shopee 让利」已过时：Garena 现仅占收入 3.1%。
