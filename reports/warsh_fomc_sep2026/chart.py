...
def signed_hbar(data, w=470, rowh=26, fmt="{:+.1f}%", label_w=None,
                pos_color=None, neg_color=None, note=None, zero_label=None):
    """带符号横条：零线居中，负值向左、正值向右。

    2026-09-15 新增。chart.py 原有的 hbar 用 `bl = bw * v / maxv` 算条长，
    负值会得到负宽度——渲染结果是**条完全消失、数值标注被挤到标签旁边粘连**，
    即「下降 9.7%」和「没有变化」在图上长得一样。那是图表模式库明令禁止的形态，
    也是 SKILL.md 逐页肉眼清单的第 1 条与第 2 条同时命中。

    diverging() 不能替代：它是为 100% 量表设计的，
    套到绝对值（亿元）上会把条压成看不见的细条且不带数值标注。
    """
    pos_color = pos_color or TEAL
    neg_color = neg_color or CLAY
    vals = [v for _, v in data]
    lo, hi = min(0, min(vals)), max(0, max(vals))
    span = (hi - lo) or 1
    if label_w is None:
        label_w = min(190, max(56, max(text_w(l, 8.4) for l, _ in data) + 10))
    _f = lambda v: fmt.format(v).replace("-", "\u2212")   # 与正文统一用减号 U+2212，不用 ASCII 连字符
    val_w = max(text_w(_f(v), 8.4) for _, v in data) + 12
    # 布局：标签 | 负值数值区 | 负条区 ‖ 正条区 | 正值数值区
    # 必须为「负值数值区」单独留 val_w——否则负条最长时数值会画进标签里，
    # 渲染成「同店9.7%」这种粘连（2026-09-15 首次渲染即命中，SKILL.md 清单第 1 条）。
    neg_pad = val_w + 8 if lo < 0 else 6
    bw = max(80, w - label_w - val_w - neg_pad - 12)
    zero_x = label_w + neg_pad + bw * (-lo) / span
    nlines = len(wrap(note, int((w - 8) / 6.2))) if note else 0
    h = rowh * len(data) + 16 + nlines * 9
    s = [f'<svg viewBox="0 0 {w} {h}" width="100%" xmlns="http://www.w3.org/2000/svg">']
    s.append(f'<line x1="{zero_x:.1f}" y1="6" x2="{zero_x:.1f}" y2="{rowh*len(data)+4}" '
             f'stroke="{INK}" stroke-width="0.9"/>')
    if zero_label:
        s.append(_txt(zero_x, rowh * len(data) + 14, zero_label, 6.6, MUTE, "middle"))
    for i, (lab, v) in enumerate(data):
        y = 8 + i * rowh
        bl = bw * abs(v) / span
        col = neg_color if v < 0 else pos_color
        x = zero_x - bl if v < 0 else zero_x
        s.append(f'<rect x="{x:.1f}" y="{y:.1f}" width="{bl:.1f}" height="{rowh-11}" fill="{col}"/>')
        s.append(_txt(label_w, y + rowh / 2 - 2, lab, 8.4, INK, "end"))
        # 数值永远标在条的外端，与左侧标签之间至少隔着零线，不可能粘连
        tx = (x - 5) if v < 0 else (x + bl + 5)
        anchor = "end" if v < 0 else "start"
        s.append(_txt(tx, y + rowh / 2 - 2, _f(v), 8.4, col, anchor, "bold"))
    if note:
        # wrap(s, n) 按字符数折行；6.6pt 下 470 单位宽约容 76 个汉字
        for j, ln in enumerate(wrap(note, int((w - 8) / 6.2))):
            s.append(_txt(4, rowh * len(data) + 15 + j * 8.4, ln, 6.6, MUTE))
    s.append("</svg>")
    return "\n".join(s)
