...
## 质量自检（设计 Agent 架构时逐条过）

```
□ 生成与验证是否由**同一个模型实例**承担？若是，这个架构已经违反核心原则。
□ Critic 是否有权否决 Builder？**没有否决权的 Critic 只是装饰。**
□ Breadcrumbs 是否记录了**决策理由**，还是只记录了动作？只记动作无法回溯。
□ Contract 是否可机器校验？**只能人读的契约在长程运行里等于没有。**
□ 失败时是否有明确的降级路径，还是默认「重试」？
```
