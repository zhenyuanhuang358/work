...
# ── 配置：改成你的报告标题 ─────────────────────────────────
REPORT_TITLE = "纪律，而非决策 · Warsh 首次加息记者会"
...
ANCHORS = [
    ("摘要", r"^\s*—\s*摘要\s*$"),
    ("1　研究对象与证据基础", r"^\s*1\s*研究对象与证据基础\s*$"),
    ("2　决策本身", r"^\s*2\s*决策本身\s*$"),
    ("3　三处范式切换", r"^\s*3\s*三处范式切换\s*$"),
    ("4　Warsh 的反应函数", r"^\s*4\s*Warsh 的反应函数\s*$"),
    ("5　三处未解决的张力", r"^\s*5\s*三处未解决的张力\s*$"),
    ("6　局限性与替代解释", r"^\s*6\s*局限性与替代解释\s*$"),
    ("7　结论：对市场的可操作含义", r"^\s*7\s*结论：对市场的可操作含义\s*$"),
    ("附录 A　参考文献与数据来源", r"^\s*A\s*参考文献与数据来源\s*$"),
    ("附录 B　关键原话摘录", r"^\s*B\s*关键原话摘录\s*$"),
]
...
def selfcheck(pdf, html=None):
    ...
    # 图表编号连续性与格式统一
    # ⚠ 2026-09-15 修：原正则只认英文 Figure，中文报告写「图 2.1」时匹配为空，
    #   于是「图表编号连续且格式统一」这项检查对中文报告从未运行过（静默失效）。
    figs = re.findall(r"(?:Figure|图)\s*([\d.]+)", full)
